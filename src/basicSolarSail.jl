...
# constructor
function basicSolarSail(; areaParam=0.0025/100, ρ=1, s=1, ϵf=0.1, ϵb=0.1, Bf=2/3, Bb=2/3, accelerationModel=:FPOF, method=:Oguri)
    ...
    # Create struct
    basicSolarSail(areaParam, ρ, s, ϵf, ϵb, Bf, Bb, condensedParams, accelerationModel, method)
end
...
function aSRP_orbitFrame(spacecraftState, u, sc::basicSolarSail, time::Float64, eph::TwoBodyEphemeride)
    #=
    INPUTS:
...
function gauss_variational_eqn!(dx, x, params, t)
    ...
    a_SRP_O = aSRP_orbitFrame(state, u, sc, epoch+t, eph);  # SRP acceleration resolved into the O (orbit) frame where O{s/c, er_hat, eθ_hat, eh_hat}
